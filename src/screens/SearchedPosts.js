...
const SearchedPosts = ({ socket, navigation, route }) => {
  const { Code, Quantity, Store } = route.params;
  ...
  const url = `/api/v1/sparePartGetTargetPosts?limit=${limit}&page=${currentPage}`;
  const { data, loading, setData } = useFetch(url, "POST", [usersData], {
    usersData: usersData,
    Code: Code,
  });
  ...
  const fetchMore = async () => {
    try {
      ...
      const fetMoreurl = `/api/v1/sparePartGetTargetPosts?limit=${limit}&page=${current}`;
      console.log(fetMoreurl);
      setListLoading(true);
      const datas = await axiosPrivate(fetMoreurl, {
        method: "POST",
        data: JSON.stringify({ username: usersData?.username, Code: Code }),
      });
  ...
  const onRefresh = async () => {
    try {
      ...
      setIsFetchMore(true);
      setRefreshing(true);
      const refreshurl = `/api/v1/sparePartGetTargetPosts?limit=${limit}&page=${current}`;
    ...
    <View style={styles.container}>
      ...
      >
        <Text style={{ fontSize: 14, fontWeight: "800" }}>{`${
          lang === "ar" ? Ar.Code : En.Code
        }: ${Code}, ${
          lang === "ar" ? Ar.Quantity : En.Quantity
        }: ${Quantity}, ${
          lang === "ar" ? Ar.Store : En.Store
        }: ${Store}`}</Text>
      </View>
      {loading ? (
