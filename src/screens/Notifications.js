...
const Notifications = ({ socket, navigation }) => {
  ...
  const url = `/api/v1/sparePartGetUserNotifications?limit=${limit}&page=${currentPage}`;
  ...
  const fetchMore = async () => {
    try {
      ...
      const fetMoreurl = `/api/v1/sparePartGetUserNotifications?limit=${limit}&page=${current}`;
  ...
  const onRefresh = async () => {
    try {
      ...
      setIsFetchMore(true);
      setRefreshing(true);
      const refreshurl = `/api/v1/sparePartGetUserNotifications?limit=${limit}&page=${current}`;
  ...
  return (
    <View style={styles.container}>
      <Navbar navigation={navigation} initial={`Notifications`} />
      {loading ? (
        ...
        <View style={{ flex: 1 }}>
          <FlatList
            ...
            renderItem={({ item }) => (
              <NotificationPost item={item} listLoading={listLoading} />
            )}
